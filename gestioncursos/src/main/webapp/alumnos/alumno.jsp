...
<%@page import="com.ipartek.formacion.pojo.Gender"%>
...
<main class="">
	...
	%>
		<a class="btn btn-warning" href="<%=Constantes.SERVLET_ALUMNOS%>">Atrás</a>
			...
			<form name="" id="" method='post' class="form-horizontal"
				...
				%>
						<div class="row">
							<div class="col-xs-3">
								<label for="">Día</label>
								<input type="number"  value="<%=calendar.get(GregorianCalendar.DAY_OF_MONTH) %>" class="form-control" min="1" max="31" step="1" name="<%=Constantes.PAR_DAY%>"	/>
							</div>
							<div class="col-xs-3">
									<label for="">Mes</label>
							...
							<div class="col-xs-6">
								<label for="">Año</label>
								<input type="number"  value="<%=calendar.get(GregorianCalendar.YEAR) %>" class="form-control" min="" max="" name="<%=Constantes.PAR_YEAR%>"	/>
							</div>
						</div>
					</div> 
				</div>
				<div class="form-group">
					...
					<div class="col-xs-10">
						<input type="radio" name="<%=Constantes.PAR_GENDER %>" id=""
							<%= Gender.MASCULINO == alumno.getGender() ? "checked" : "" %> 
							 value="<%=Gender.MASCULINO.getCodigo() %>"/> 
							<%=Gender.MASCULINO.getValor() %>
						<input type="radio" name="<%=Constantes.PAR_GENDER %>" id="" 
							<%= Gender.FEMENINO == alumno.getGender() ? "checked" : "" %>
							value="<%=Gender.FEMENINO.getCodigo() %>"/> 
							<%=Gender.FEMENINO.getValor() %>
						<input type="radio" name="<%=Constantes.PAR_GENDER %>" id="" 
							<%= Gender.OTROS == alumno.getGender() ? "checked" : "" %>
							value="<%=Gender.OTROS.getCodigo() %>"/> 
							<%=Gender.OTROS.getValor() %>															
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-xs-10">
						...
						<input type="checkbox" name="<%=Constantes.PAR_IDIOMA %>" id="" 
							<%= alumno.getIdiomas().contains(Idioma.ENGLISH)  ? "checked" : "" %>
							value="<%=Idioma.ENGLISH.getCodigo() %>"/> 
							<%=Idioma.ENGLISH.getNombre() %>															
					</div>
					<div class="form-group">
