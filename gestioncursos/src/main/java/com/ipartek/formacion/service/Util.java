...
import com.ipartek.formacion.pojo.Gender;
...
public class Util {
	...
	public static Gender parseGender(String gender){
		Gender aux = Gender.MASCULINO;
		int codigo = Integer.parseInt(gender);
		if(codigo== Gender.FEMENINO.getCodigo()){
			aux = Gender.FEMENINO;
		}
		return aux;
	}
	...
	public static List<Idioma> parseIdioma(String[] idiomas){
		...
		for (String idioma2 : idiomas) {
			...
			if(codigoIdioma == Idioma.EUSKERA.getCodigo()){
				...
				if(codigoIdioma==Idioma.ENGLISH.getCodigo()){
