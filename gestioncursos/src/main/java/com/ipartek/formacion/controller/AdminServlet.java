...
/**
 * @author Anabel Servlet implementation class AdminServlet
 */
public class AdminServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (request.getParameter(Constantes.PAR_IDSESSION) != null) {
      this.expulsarUsuario(request);
    }
    this.cargarUsuario(request, response);
  }
  ...
  private void expulsarUsuario(HttpServletRequest request) {
    ...
    Map<String, HttpSession> sesiones = (Map<String, HttpSession>) context
        .getAttribute(Constantes.ATT_SESIONES);
  ...
  private void cargarUsuario(HttpServletRequest request, HttpServletResponse response) {
    this.rd = request.getRequestDispatcher(Constantes.JSP_ADMIN);
    try {
